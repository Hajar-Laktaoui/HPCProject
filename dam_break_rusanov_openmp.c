...
// Initialization of conditions (discontinuous)
...
void rusanov_flux(int nc, double *h, double *u, double flux[2][NX - 1]) {
    ...
    for (int i = 0; i < nc - 1; ++i) {
        //Aucune écriture sur h[] ou u[] → pas de conflit
...
int main() {
    ...
    double x[NX], xc[NX - 1];
    double h[NX - 1], u[NX - 1];
    double W[2][NX - 1], Wn[2][NX - 1], flux[2][NX - 1];

    // Grid initialization (parallelized)
    #pragma omp parallel for
    for (int i = 0; i < nx; ++i)
        x[i] = XLEFT + i * dx;
    ...
    double t = 0.0;
    while (t < TEND) {
        // Velocity and wave speed calculation with reduction
        ...
        // Time step calculation (serial)
        double dt = CFL * dx / max_speed;
        if (t + dt > TEND)
            dt = TEND - t;
        t += dt;

        // Flux calculation (parallel)
        ...
        for (int i = 1; i < nc - 1; ++i) {
            for (int j = 0; j < 2; ++j)
                Wn[j][i] = W[j][i] - (dt/dx) * (flux[j][i] - flux[j][i - 1]);
        }
        ...
        // Optional progress output (serial)
        #pragma omp master
        {
            printf("t = %.4f\n", t);
        }
    ...
    printf("Results written to output_omp.dat\n");
